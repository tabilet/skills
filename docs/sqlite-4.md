...
| SQL4-T04 | [X] | Add context-archive snapshots with independent ID, baseline, and privacy tests. Cancelled when new full-file capture was deferred. | Verified archives remain immutable and archive IDs stay separate from status IDs. |
...
This historical milestone is superseded. Its host and read-only interfaces remain
accepted; SQL5–8 own the revised release contract and explicitly cancel new
context-archive snapshot capture.
