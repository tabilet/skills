...
(function () {
  ...
  function timelineEntry(item, child) {
    ...
    entry.append(el("time", { datetime: exact || "", title: exact || "", text: when })); const content = el("div"); content.append(el("h3", {}, [item.operation || "Recorded operation"])); content.append(el("p", { text: item.request_summary || "Request text was not captured." })); content.append(el("p", { class: "meta", text: item.result_summary || item.result || (item.unfinished ? "Unfinished run" : "No result summary recorded.") })); if (item.legacy) content.append(badge("Evidence: legacy", "capture")); else { content.append(badge(`Capture: ${item.capture_fidelity || "incomplete"}`, "capture")); if (!item.provenance) content.append(badge("Provenance not supplied", "capture")); } if (item.provenance) { content.append(el("p", { class: "meta", text: `Instruction: ${item.provenance.instruction_set_name || "legacy"}${item.provenance.instruction_set_version ? `@${item.provenance.instruction_set_version}` : ""} · ${item.provenance.capture_method || "legacy"} · fingerprint ${item.provenance.fingerprint_fidelity === "unavailable" ? "fingerprint unavailable" : item.provenance.fingerprint_fidelity}` })); } if (item.coverage) content.append(badge(`Coverage: ${item.coverage.coverage || "missing"}${item.coverage.content_state === "purged" || item.coverage.content_state === "partially_purged" ? ` · ${item.coverage.content_state}` : ""}`, "capture")); entry.append(content); entry.append(button("Open", () => openRun(item.run_id), { "aria-label": `Open ${item.operation || "run"} details` }));
  ...
  function renderTodo(data) {
    const body = obj(data); const validated = body.validated !== false; const totals = obj(body.totals); const pages = obj(body.pagination); const anyTasks = todoGroups.some((key) => Number(totals[key] == null ? list(body[key]).length : totals[key]) > 0); $("todo-validation").textContent = validated ? "Indexed evidence · verify live ledger" : "Indexed evidence may be stale"; const root = $("todo-content"); clear(root); if (body.validation_reason) root.append(el("p", { class: "diagnostic", text: body.validation_reason })); if (anyTasks) root.append(el("p", { class: "muted" }, ["These groups are advisory. Verify the live ledger before selecting work."])); const groups = [["Resume", "resume"], ["Ready", "ready"], ["Waiting", "waiting"], ["Blocked", "blocked"], ["Needs review", "needs_review"]]; groups.forEach(([title, key]) => { const records = list(body[key]); const page = obj(pages[key]); const total = totals[key] == null ? records.length : Number(totals[key]); const section = el("section", { class: "todo-group" }, [el("h3", {}, [title, badge(String(total))])]); if (!records.length) section.append(el("p", { class: "muted" }, ["None"])); else { const ordered = el(key === "ready" ? "ol" : "ul", { class: "todo-list", start: key === "ready" ? Number(page.offset || 0) + 1 : null }); records.forEach((item) => { const canAct = validated && Boolean(item.source && item.source.path) && (key === "needs_review" || Boolean(item.source.sha256)); ordered.append(todoItem(item, key, canAct)); }); section.append(ordered); } if (page.offset > 0 || page.more) { const controls = el("div", { class: "pagination", "aria-label": `${title} pagination` }); if (page.offset > 0) controls.append(button("Previous", () => loadTodo(key, Math.max(0, page.offset - page.limit), true))); if (page.more) controls.append(button("Next", () => loadTodo(key, page.offset + page.limit, true))); section.append(controls); } root.append(section); });
  }
  ...
  function messageRow(message) { const state = message.content_state || (message.text ? "available" : "not captured"); const text = state === "purged" ? "Message content purged; envelope and hash retained." : state === "not captured" ? "Message text was not captured." : message.text || ""; return el("article", { class: "recorded-message" }, [el("strong", { text: message.role || "message" }), " ", badge(message.fidelity || "incomplete"), badge(state), el("pre", { text })]); }
  async function openRun(runId, push, offsets) {
    ...
    try {
      ...
      if (run.legacy) root.append(detailSection("Audit evidence", [el("p", { text: "Legacy audit record: this run predates SQLite 13 provenance and capture-coverage evidence." })])); else if (!run.provenance) root.append(detailSection("Invocation provenance", [el("p", { text: "Provenance not supplied for this run." })])); else if (run.provenance) root.append(detailSection("Invocation provenance", [el("p", { text: `Instruction set: ${run.provenance.instruction_set_name || "legacy"}${run.provenance.instruction_set_version ? `@${run.provenance.instruction_set_version}` : ""}` }), el("p", { text: `Host: ${run.provenance.host_agent || "unavailable"} · Model: ${run.provenance.model || "unavailable"}` }), el("p", { text: `Capture: ${run.provenance.capture_method || "legacy"} · Fingerprint: ${run.provenance.fingerprint_fidelity === "unavailable" ? "fingerprint unavailable" : run.provenance.fingerprint_fidelity}` })]));
      if (list(run.coverage).length) root.append(detailSection("Capture coverage", list(run.coverage).map((item) => el("p", { text: `${item.scope}: ${item.coverage} · ${item.content_state} (exact ${item.exact_count || 0}, redacted ${item.redacted_count || 0}, summarized ${item.summarized_count || 0}, incomplete ${item.incomplete_count || 0})` }))));
      const requestState = request.content_state || (request.text ? "available" : "not captured"); const outputState = output.content_state || (output.text ? "available" : "not captured");
      root.append(detailSection("Recorded then — request", [el("p", { text: requestState === "purged" ? "Request content purged; envelope and hash retained." : request.text || "Request text was not captured." }), badge(request.fidelity || "incomplete"), badge(requestState)]));
      root.append(detailSection("Recorded then — output", [el("pre", { text: outputState === "purged" ? "Output content purged; envelope and hash retained." : output.text || "No selected output was recorded." }), badge(output.fidelity || "incomplete"), badge(outputState)]));
  ...
  function restoreUrlState() {
    const params = new URLSearchParams(location.search); state.timelineFilters = { operation: params.get("operation") || "", outcome: params.get("outcome") || "", milestone: params.get("milestone") || "", instruction_set: params.get("instruction_set") || "", instruction_set_version: params.get("instruction_set_version") || "", host: params.get("host") || "", model: params.get("model") || "", capture_method: params.get("capture_method") || "", coverage: params.get("coverage") || "", purged: params.get("purged") || "", since: params.get("since") || "", until: params.get("until") || "", order: params.get("order") || "newest" }; todoGroups.forEach((key) => { state.todoOffsets[key] = Math.max(0, Number(params.get(`${key}_offset`)) || 0); }); ["operation", "outcome", "milestone", "instruction-set", "instruction-set-version", "host", "model", "capture-method", "coverage", "since", "until", "order"].forEach((name) => { const key = name.replaceAll("-", "_"); const node = $("timeline-" + name); if (node) node.value = state.timelineFilters[key]; }); $("timeline-purged").checked = state.timelineFilters.purged === "1"; $("global-search").value = params.get("search") || ""; setView(params.get("view") || "overview", true); if (params.get("run")) openRun(params.get("run"), false, { event_offset: Number(params.get("event_offset")) || 0, message_offset: Number(params.get("message_offset")) || 0, child_offset: Number(params.get("child_offset")) || 0 }); else if (params.get("document")) openDocument(params.get("document"), Number(params.get("line")) || null, false); else if (params.get("search")) openSearch(params.get("search"), false, Number(params.get("search_offset")) || 0); else closePanel(false);
  }
  function setup() {
    restoreUrlState(); document.querySelectorAll("[data-view]").forEach((link) => link.addEventListener("click", (event) => { event.preventDefault(); setView(link.dataset.view, false); })); window.addEventListener("popstate", restoreUrlState); document.addEventListener("keydown", (event) => { if (event.key === "Escape" && !$("detail-panel").hidden) { event.preventDefault(); closePanel(true); } }); $("close-detail").addEventListener("click", () => closePanel(true)); $("backdrop").addEventListener("click", () => closePanel(true)); $("refresh-project").addEventListener("click", refresh); ["operation", "outcome", "milestone", "instruction-set", "instruction-set-version", "host", "model", "capture-method", "coverage", "since", "until"].forEach((name) => { const node = $("timeline-" + name); node.addEventListener("change", () => { const key = name.replaceAll("-", "_"); state.timelineFilters[key] = node.value.trim(); updateUrl({ [key]: state.timelineFilters[key], cursor: null }, true); loadTimeline(); }); }); $("timeline-purged").addEventListener("change", () => { state.timelineFilters.purged = $("timeline-purged").checked ? "1" : ""; updateUrl({ purged: state.timelineFilters.purged || null, cursor: null }, true); loadTimeline(); }); $("timeline-order").addEventListener("change", () => { state.timelineFilters.order = $("timeline-order").value; updateUrl({ order: state.timelineFilters.order === "newest" ? null : state.timelineFilters.order, cursor: null }, true); loadTimeline(); }); $("global-search").addEventListener("keydown", (event) => { if (event.key === "Enter" && event.currentTarget.value.trim()) { event.preventDefault(); openSearch(event.currentTarget.value.trim()); } }); loadCurrent();
