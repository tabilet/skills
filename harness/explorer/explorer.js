...
(function () {
  ...
  async function openRun(runId, push, offsets) {
    ...
    if (push !== false) updateUrl({ run: runId }, false);
    ...
    try {
      ...
      if (conversation.length) {
        ...
        const controls = pager("Messages", pagination.messages, (value) => openRun(runId, false, { ...pageOffsets, message_offset: value }));
        if (controls) section.append(controls); root.append(section);
      }
      if (list(body.events).length) {
        ...
        const controls = pager("Events", pagination.events, (value) => openRun(runId, false, { ...pageOffsets, event_offset: value }));
        if (controls) section.append(controls); root.append(section);
      }
      ...
      if (list(body.children).length) {
        ...
        const controls = pager("Child operations", pagination.children, (value) => openRun(runId, false, { ...pageOffsets, child_offset: value }));
        if (controls) section.append(controls); root.append(section);
      }
  ...
  function closePanel(updateHistory) { $("detail-panel").hidden = true; $("backdrop").hidden = true; if (updateHistory !== false && state.detailParameter) { const values = { [state.detailParameter]: null }; if (state.detailParameter === "document") values.line = null; updateUrl(values, true); } state.detailParameter = null; if (state.returnFocus && typeof state.returnFocus.focus === "function") state.returnFocus.focus(); }
  function restoreUrlState() {
    const params = new URLSearchParams(location.search); state.timelineFilters = { operation: params.get("operation") || "", outcome: params.get("outcome") || "", milestone: params.get("milestone") || "", since: params.get("since") || "", until: params.get("until") || "", order: params.get("order") || "newest" }; $("timeline-operation").value = state.timelineFilters.operation; $("timeline-outcome").value = state.timelineFilters.outcome; $("timeline-milestone").value = state.timelineFilters.milestone; $("timeline-since").value = state.timelineFilters.since; $("timeline-until").value = state.timelineFilters.until; $("timeline-order").value = state.timelineFilters.order; $("global-search").value = params.get("search") || ""; setView(params.get("view") || "overview", true); if (params.get("run")) openRun(params.get("run"), false); else if (params.get("document")) openDocument(params.get("document"), Number(params.get("line")) || null, false); else if (params.get("search")) openSearch(params.get("search"), false); else closePanel(false);
  }
  function setup() {
