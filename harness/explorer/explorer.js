...
(function () {
  ...
  async function openRun(runId, push, offsets) {
    ...
    try {
      ...
      if (run.legacy) root.append(detailSection("Audit evidence", [el("p", { text: "Legacy audit record: this run predates SQLite 13 provenance and capture-coverage evidence." })])); else if (run.provenance) root.append(detailSection("Invocation provenance", [el("p", { text: `Instruction set: ${run.provenance.instruction_set_name || "legacy"}${run.provenance.instruction_set_version ? `@${run.provenance.instruction_set_version}` : ""}` }), el("p", { text: `Host: ${run.provenance.host_agent || "unavailable"} · Model: ${run.provenance.model || "unavailable"}` }), el("p", { text: `Capture: ${run.provenance.capture_method || "legacy"} · Fingerprint: ${run.provenance.fingerprint_fidelity || "unavailable"}` })]));
