...
(function () {
  ...
  function sourcePreview(value, line) { const content = string(value); if (!line) return el("pre", { class: "source-document" }, [content]); const lines = content.split("\n"); const target = Math.min(lines.length, Math.max(1, Number(line))); const start = Math.max(1, target - 5); const end = Math.min(lines.length, target + 5); const width = String(end).length; const excerpt = lines.slice(start - 1, end).map((entry, index) => { const number = start + index; return `${number === target ? ">" : " "} ${String(number).padStart(width, " ")} | ${entry}`; }).join("\n"); return el("pre", { class: "source-document", "data-line": target }, [excerpt]); }
