...
(function () {
  "use strict";
  const state = { view: "overview", timelineNext: null, timelinePrevious: null, timelineFilters: {}, generation: null, activity: null, poll: null, returnFocus: null, detailParameter: null };
  ...
  function source(ref) {
    ...
    return button(label, () => openDocument(path), { class: "source source-button", "aria-label": `Open ${label}` });
  }
  ...
  function renderOverview(data) {
    ...
    if (list(body.attention).length) {
      ...
      list(body.attention).forEach((item) => { const row = el("li", { class: "attention-item" }, [el("strong", { text: item.title || item.kind || "Attention" }), el("p", { text: item.message || item.reason || "" })]); if (item.source) row.append(source(item.source)); attention.append(row); }); root.append(attention);
    }
  ...
  function renderTodo(data) {
    const body = obj(data); const validated = body.validated !== false; const recommendations = body.recommendations_available !== false; $("todo-validation").textContent = validated && recommendations ? "Validated recommendations" : "Evidence available; recommendations withheld"; const root = $("todo-content"); clear(root); if (body.validation_reason) root.append(el("p", { class: "diagnostic", text: body.validation_reason })); if (!recommendations) root.append(el("p", { class: "muted" }, ["Current evidence remains available below. Resolve review or freshness requirements before continuing execution."])); const groups = [["Resume", "resume"], ["Ready", "ready"], ["Waiting", "waiting"], ["Blocked", "blocked"], ["Needs review", "needs_review"]]; groups.forEach(([title, key]) => { const records = list(body[key]); const section = el("section", { class: "todo-group" }, [el("h3", {}, [title, badge(String(records.length))])]); if (!records.length) section.append(el("p", { class: "muted" }, ["None"])); else { const ordered = el(key === "ready" ? "ol" : "ul", { class: "todo-list" }); records.forEach((item) => { const canAct = validated && (key === "needs_review" ? Boolean(item.milestone_id || item.source) : ["blocked", "waiting"].includes(key) ? !list(body.needs_review).length : recommendations); ordered.append(todoItem(item, key, canAct)); }); section.append(ordered); } root.append(section); });
  }
  ...
  function openPanel(title, content) { state.returnFocus = document.activeElement; $("detail-title").textContent = title; const root = $("detail-content"); clear(root); root.append(content); $("detail-panel").hidden = false; $("backdrop").hidden = false; $("close-detail").focus(); }
  ...
  async function openDocument(path) { state.detailParameter = null; try { const body = await api(`/api/document?path=${encodeURIComponent(path)}&live=1`); openPanel(`Source: ${path}`, el("pre", { class: "source-document" }, [body.text || ""])); } catch (error) { openPanel("Source unavailable", el("p", { class: "diagnostic", text: error.message })); } }
  async function openRun(runId, push) {
    if (!runId) return; state.detailParameter = "run"; if (push !== false) updateUrl({ run: runId }, false);
    try { const body = await api(`/api/runs/${encodeURIComponent(runId)}`); const run = obj(body.run); const messages = list(body.messages); const request = messages.find((message) => ["user", "request"].includes(message.role)); const output = messages.filter((message) => ["assistant", "output"].includes(message.role)).pop(); const root = el("div"); root.append(detailSection("Recorded then — request", [el("p", { text: request ? request.text : "Request text was not captured." }), badge(request ? request.fidelity : "incomplete")])); root.append(detailSection("Recorded then — output", [el("pre", { text: output ? output.text : "No selected output was recorded." })])); if (list(body.events).length) root.append(detailSection("Recorded then — observed activity", list(body.events).map((event) => { const details = eventDetails(event); const explorer = obj(event.explorer); const summary = event.summary || explorer.summary || details.summary || details.raw || JSON.stringify(details); const row = el("p", {}, [el("strong", { text: explorer.phase || event.event_type || "event" }), " — ", summary]); if (event.status_path) row.append(" ", source({ path: event.status_path })); list(explorer.artifact_refs).forEach((artifact) => { if (artifact.path) row.append(" ", source(artifact)); }); return row; }))); const current = list(body.current_state).map((item) => { if (!item.resolved) return el("p", { class: "diagnostic", text: `${item.kind} ${item.identifier}: ${item.reason || "current location unresolved"}` }); const record = obj(item.record); return el("p", {}, [`${item.kind} ${item.identifier}${record.state ? ` — ${record.state}` : ""} `, record.path ? source({ path: record.path, line: record.line }) : ""]); }); root.append(detailSection("Current state", current.length ? current : [el("p", { class: "muted", text: "No referenced current milestone or task could be resolved." })])); openPanel(run.operation || "Run details", root); }
  ...
  window.TabiletExplorerTest = { activityChanged, renderTodo, timelineEntry, restoreUrlState, updateUrl, closePanel, openPanel, prepareFollowUp };
