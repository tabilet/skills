...
コーディングエージェントは、プロジェクトが自分自身を説明できるとうまく動きます。これが何で、何が終わっていて、次が何かということです。このリポジトリは、まさにそれを担う小さなプレーンテキストファイル群を提供します。大半は markdown です。プロジェクトにコピーすれば、その瞬間からあなたのものになります。

ここにあるものはすべてプレーンテキストなので、必要な道具は `git` だけです。ファイルはそのまま読めますし、手で編集し、名前を変え、削除できます。markdown を読めるエージェントならどれでも扱えます。任意のプラグインがファイルを生成してくれますし、任意の API runner が無人で処理を進めてくれますが、どちらもプロジェクトの外側にとどまります。

`template/` はプロジェクトへ、`harness/` は API runner が必要なときに home ディレクトリへコピーします。ファイルが揃えばそれはプロジェクトのものになり、プロジェクト自体はこのリポジトリから独立したままです。半年後も、保守しているのは自分のコードだけです。

3 つの任意の skill が、そのコピーと記入を代行できます（[3 つのコマンドをインストールする](#3-つのコマンドをインストールする)を参照）。それらが書いたファイルは現れた瞬間からあなたのものであり、あなたが残したままの姿を保ちます。
...
本文を通じて **harness** とは、何かが確かに動くことを証明する、繰り返し実行可能なコマンドを指します。テストスイート、CI job、スクリプトなどです。プロジェクトは自分の harness を `tech-stack.md` に定義します。このリポジトリはさらに任意の harness を 1 つ同梱しています。API 経由でエージェントを動かし、memory bank を無人で進めるループです。
...
memory bank に必要なのは `git` だけです。ただの markdown なので、日々のワークフローにランタイムは要りません。Codex や Claude Code のようなエージェントに次の未処理項目を任せれば、エージェントがファイルを直接編集します。
...
いちばん手軽な始め方に clone は要りません。プラグインをインストールし、名前空間付きの `memory-bank-init` skill に質問させて、memory bank を書いてもらいます:
...
使うことを求めてはいません。`/goal` はあなたのエージェントのコマンドであって、この harness のものではありません。自前の protocol を持ち込んでも、何も使わなくても、memory bank のふるまいは変わりません。`GOAL.md` を同梱しているのは、この種の protocol を書くのが面倒だからであって、ここにある何かがそれに依存しているからではありません。自前のものがあるなら、`GOAL.md` に触れている 2 か所をそちらに向けるか、削除してください。その 2 か所は `AGENTS.md` と `memory-bank/milestone.md` です。
...
この 2 つは競合ではなく補完です。プロジェクトを生まない決定には `/grill-me` を使い続けてください。アーキテクチャの議論、採用計画、講演の構成などです。grill の対象が、来週も自分が何であるかを知っている必要のあるコードベースなら `memory-bank-init` です。
